...
class BiEncoderRanker(torch.nn.Module):
    ...
    def score_candidate(
        ...
        # print("Encoding context", device)
        # print(token_idx_ctxt, segment_idx_ctxt, mask_ctxt)
        # print(self.device)
        embedding_ctxt, _ = self.model(
            token_idx_ctxt.to(device), segment_idx_ctxt.to(device), mask_ctxt.to(device), None, None, None
        ...
        if cand_encs is not None:
            # print(embedding_ctxt.get_device(), cand_encs.get_device())
            # print("calculating", device)
        ...
        _, embedding_cands = self.model(
            None, None, None, token_idx_cands.to(device), segment_idx_cands.to(device), mask_cands.to(device)
            # None, None, None, token_idx_cands, segment_idx_cands, mask_cands
        )
        if random_negs:
